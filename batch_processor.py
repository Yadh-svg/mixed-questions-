...
def group_questions_by_type_and_topic(questions_config: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for q_type, type_questions in grouped_by_type.items():
        # User requested Strict Input Order Preservation.
        # Disabled "Priority Packing", topic grouping, and alphabetical sorting.
        logger.info(f"  - {q_type}: {len(type_questions)} questions (Preserving Strict User Input Order)")
        final_grouped[q_type] = type_questions

    logger.info(f"Grouped {len(questions_config)} questions into {len(final_grouped)} types (Order Maintained).")
